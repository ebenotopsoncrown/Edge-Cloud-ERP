...
import { Account, JournalEntry, Product, Customer, Invoice, Vendor, Bill } from "@/api/entities";
...
export default function AccountingDemo() {
  ...
  const { data: journalEntries = [] } = useQuery({
    ...
    queryFn: () => currentCompany ? JournalEntry.list({ filters: { company_id: currentCompany.id }, orderBy: 'created_date', ascending: false }) : [],
    enabled: !!currentCompany
  });
  ...
  const runFullTest = async () => {
    ...
    try {
      ...
      const openingJE = await JournalEntry.list({ filters: {
        company_id: currentCompany.id,
      ...
      if (openingJE.length > 0) {
        ...
        if (inventoryAccount && equityAccount) {
          const invAccount = await Account.get(inventoryAccount.id);
          const eqAccount = await Account.get(equityAccount.id);
      ...
      const invoiceJEs = await JournalEntry.list({ filters: {
        company_id: currentCompany.id,
      ...
      const updatedProduct = await Product.get(product.id);
      addTestResult({ 
        type: 'info', 
      ...
      // Check journal entries
      await new Promise(resolve => setTimeout(resolve, 1000));
      const billJEs = await JournalEntry.list({ filters: {
        company_id: currentCompany.id,
        source_id: bill.id,
        source_type: 'bill'
      } });
      ...
      const finalProduct = await Product.get(product.id);
      addTestResult({ 
        type: 'info', 
      ...
      const allAccounts = await Account.list({ filters: { company_id: currentCompany.id } });
