...
export default function ReceiveInventory() {
  ...
  const { data: receipts = [], isLoading } = useQuery({
    ...
    queryFn: () => currentCompany ? InventoryReceipt.list({ filters: { company_id: currentCompany.id }, orderBy: 'receipt_date', ascending: false }) : Promise.resolve([]),
  ...
  const { data: pendingPOs = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const pos = await PurchaseOrder.list({ filters: {
