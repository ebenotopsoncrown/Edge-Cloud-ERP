...
import { Vendor, PurchaseOrder, Account, InventoryLocation, InventoryReceipt, Bill, JournalEntry, Product, InventoryTransaction } from "@/api/entities";
...
export default function ReceiveInventoryForm({ onClose, pendingPOs = [] }) {
  ...
  const { data: vendorPOs = [] } = useQuery({
    ...
    queryFn: () => selectedVendorId ? PurchaseOrder.list({ filters: {
      company_id: currentCompany.id,
  ...
  const { data: selectedPO } = useQuery({
    ...
    queryFn: () => selectedPOId ? PurchaseOrder.get(selectedPOId) : null,
  ...
  const { data: locations = [] } = useQuery({
    ...
    queryFn: () => currentCompany ? InventoryLocation.list({ filters: { company_id: currentCompany.id } }) : [],
    enabled: !!currentCompany
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const savedReceipt = await InventoryReceipt.create(receiptData);
      ...
      await InventoryReceipt.update(savedReceipt.id, {
      ...
      try {
        const vendor = await Vendor.get(data.vendor_id);
      ...
      await Account.update(apAccount.id, {
      ...
      await Bill.update(savedBill.id, {
      ...
      if (data.po_id) {
        ...
        const po = await PurchaseOrder.get(data.po_id);
        ...
        await PurchaseOrder.update(data.po_id, {
