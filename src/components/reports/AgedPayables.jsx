...
export default function AgedPayables({ bills, baseCurrency = 'USD' }) {
  const agedData = useMemo(() => {
    ...
    // Calculate aged amounts from unpaid bills
    bills
      .filter(bill => ['pending', 'partial', 'overdue'].includes(bill.status))
      .forEach(bill => {
        const dueDate = new Date(bill.due_date);
        const daysOverdue = Math.floor((today - dueDate) / (1000 * 60 * 60 * 24));
        const outstandingAmount = bill.balance_due || bill.total_amount;
  ...
  return (
    <Card className="shadow-md border-none">
      <CardHeader className="bg-gray-50 border-b">
        <CardTitle className="text-lg">Aged Payables</CardTitle>
      </CardHeader>
