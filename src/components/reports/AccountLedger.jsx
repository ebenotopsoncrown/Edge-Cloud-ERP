...
import { JournalEntry, Invoice, Bill, Account } from "@/api/entities";
...
export default function AccountLedger({ account, onClose, onTransactionClick }) {
  ...
  const { data: journalEntries = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const allEntries = await JournalEntry.list({ filters: {
        company_id: currentCompany.id,
        status: 'posted'
      }, orderBy: 'entry_date', ascending: false });
  ...
  const handleTransactionClick = async (entry) => {
    ...
    try {
      ...
      if (entry.source_type === 'invoice' && entry.source_id) {
        ...
        try {
          const invoice = await Invoice.get(entry.source_id);
        ...
        try {
          const bill = await Bill.get(entry.source_id);
