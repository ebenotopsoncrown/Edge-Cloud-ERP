...
import { Vendor, Account, JournalEntry } from "@/api/entities";
...
export default function VendorForm({ vendor, onClose, companyId }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (vendor?.id) {
        savedVendor = await Vendor.update(vendor.id, data);
      } else {
        savedVendor = await Vendor.create(data);
        ...
        if (!vendor && openingBalance > 0) {
          ...
          if (apAccount && equityAccount) {
            ...
            await Vendor.update(savedVendor.id, {
