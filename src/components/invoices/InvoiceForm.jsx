...
import { Invoice, Customer, Product, Account, Job, JournalEntry, InventoryTransaction } from "@/api/entities";
...
export default function InvoiceForm({ invoice, invoiceId, onClose }) {
  ...
  const { data: existingInvoice } = useQuery({
    ...
    queryFn: () => invoiceId ? Invoice.get(invoiceId) : null,
    enabled: !!invoiceId
  });
  ...
  const { data: jobs = [] } = useQuery({
    ...
    queryFn: () => currentCompany ? Job.list({ filters: { company_id: currentCompany.id } }) : [],
    enabled: !!currentCompany
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (invoiceId || invoice?.id) {
        ...
        oldInvoice = await Invoice.get(invoiceId || invoice.id);
        savedInvoice = await Invoice.update(invoiceId || invoice.id, invoiceData);
      } else {
      ...
      if (data.status === 'sent' || data.status === 'paid') {
        ...
        if (isEditing && oldInvoice) {
          ...
          const allJournalEntries = await JournalEntry.list();
          ...
          for (const je of relatedEntries) {
            ...
            await JournalEntry.delete(je.id);
        ...
        const je1 = await JournalEntry.create(journalEntry1);
        ...
        await Account.update(arAccount.id, {
        ...
        await Account.update(revenueAccount.id, {
        ...
        if (taxAccount && data.tax_total > 0) {
          ...
          await Account.update(taxAccount.id, {
          ...
          if (item.product_id) {
            try {
              ...
              if (product && product.product_type === 'inventory') {
                ...
                await InventoryTransaction.create(inventoryTransaction);
              }
            } catch (error) {
        ...
        if (totalCOGS > 0 && cogsAccount && inventoryAccount) {
          ...
          const je2 = await JournalEntry.create(journalEntry2);
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      const fullInvoice = await Invoice.get(invoiceToDelete.id || invoiceToDelete);
        ...
        for (const item of fullInvoice.line_items) {
          if (item.product_id) {
            try {
              ...
              const relatedInvTransactions = await InventoryTransaction.list({ filters: {
                reference_id: fullInvoice.id,
                product_id: item.product_id,
                transaction_type: 'sale'
              } });
              for (const trans of relatedInvTransactions) {
                await InventoryTransaction.delete(trans.id);
      ...
      try {
        ...
        await Invoice.delete(fullInvoice.id);
