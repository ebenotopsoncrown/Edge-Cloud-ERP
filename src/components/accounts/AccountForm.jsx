...
export default function AccountForm({ account, onClose }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (account?.id) {
        savedAccount = await Account.update(account.id, data);
      } else {
        savedAccount = await Account.create(data);
      }
      ...
      if (!account?.id && openingBalance !== 0 && canHaveOpeningBalance) {
        const equityAccount = await Account.list({ filters: {
          company_id: currentCompany.id,
          account_type: 'equity',
        ...
        if (equityAccount.length > 0) {
          ...
          await Account.update(savedAccount.id, {
          ...
          await Account.update(equityAccount[0].id, {
