...
export default function CustomerForm({ customer, onClose, companyId }) {
  ...
  const { data: accounts = [] } = useQuery({
    ...
    queryFn: () => companyId ? Account.list({ filters: { company_id: companyId } }) : Promise.resolve([]),
    enabled: !!companyId
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (customer?.id) {
        savedCustomer = await Customer.update(customer.id, data);
      } else {
        savedCustomer = await Customer.create(data);
        ...
        if (!customer && openingBalance > 0) {
          ...
          if (arAccount && equityAccount) {
            ...
            await Customer.update(savedCustomer.id, {
