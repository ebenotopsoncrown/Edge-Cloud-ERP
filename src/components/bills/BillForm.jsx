...
import { Bill, Vendor, Product, Account, Job, JournalEntry } from "@/api/entities";
...
export default function BillForm({ bill, billId, onClose }) {
  ...
  const { data: existingBill } = useQuery({
    ...
    queryFn: () => billId ? Bill.get(billId) : null,
    enabled: !!billId
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (billId || bill?.id) {
        isEditing = true;
        oldBill = await Bill.get(billId || bill.id);
        savedBill = await Bill.update(billId || bill.id, billData);
      } else {
      ...
      if (data.status === 'pending' || data.status === 'approved') { // Should be 'pending' or 'approved'
        ...
        if (isEditing && oldBill) {
          ...
          if (oldBill.journal_entry_id) {
            try {
              const oldJE = await JournalEntry.get(oldBill.journal_entry_id);
              ...
              await JournalEntry.delete(oldBill.journal_entry_id);
        ...
        if (debitAccount) {
          ...
          await Account.update(debitAccount.id, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      const fullBill = await Bill.get(billToDelete.id || billToDelete);
      ...
      const allJournalEntries = await JournalEntry.list({ filters: {
