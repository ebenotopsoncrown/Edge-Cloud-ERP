...
import { Customer, Account, JournalEntry } from "@/api/entities";
...
export default function ImportCustomers() {
  ...
  const parseMutation = useMutation({
    mutationFn: async (fileUrl) => {
      ...
      return result.output.customers || result.output;
    },
    onSuccess: (data) => {
  ...
  const validateData = (data) => {
    ...
    const results = data.map((row, index) => {
      ...
      if (row.customer_code && existingCustomers.some(c => c.customer_code === row.customer_code)) {
      ...
      for (const { row } of validRows) {
        try {
          ...
          const customerData = {
            ...
            customer_code: row.customer_code || `CUST${Date.now()}`,
          ...
          // Post opening balance journal if balance exists
          if (openingBalance > 0 && arAccount && equityAccount) {
            const journalEntry = {
              company_id: currentCompany.id,
              entry_number: `OB-CUST-${customerData.customer_code}`,
              entry_date: new Date().toISOString().split('T')[0],
              reference: `Customer Opening Balance - ${customerData.company_name}`,
              source_type: 'manual',
              description: `Opening Balance - ${customerData.company_name}`,
              status: 'posted',
              line_items: [
                {
                  ...
                  description: `Customer opening balance - ${customerData.company_name}`,
            ...
            await Account.update(equityAccount.id, {
      ...
      {importResults && (
        <Alert className="bg-blue-50 border-blue-200">
          ...
          <AlertDescription className="text-blue-900">
            ...
            <p className="mt-2">
              Successfully imported: {importResults.success} customers<br />
              Failed: {importResults.failed} customers
            </p>
              ...
              <div className="mt-3">
                ...
                <ul className="list-disc ml-5 mt-1">
                  {importResults.errors.map((err, i) => (
                    <li key={i}>{err.customer}: {err.error}</li>
                  ))}
                </ul>
              </div>
            )}
