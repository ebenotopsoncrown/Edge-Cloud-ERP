...
import { Customer, Invoice, Account, SalesReturn, JournalEntry, Product, InventoryTransaction } from "@/api/entities";
...
export default function SalesReturnForm({ onClose }) {
  ...
  const { data: customerInvoices = [] } = useQuery({
    ...
    queryFn: () => selectedCustomerId ? Invoice.list({ filters: {
      company_id: currentCompany.id,
  ...
  const { data: selectedInvoice } = useQuery({
    ...
    queryFn: () => selectedInvoiceId ? Invoice.get(selectedInvoiceId) : null,
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const savedReturn = await SalesReturn.create(returnData);
