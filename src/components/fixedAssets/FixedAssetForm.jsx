...
import { FixedAsset, Vendor, Account, JournalEntry } from "@/api/entities";
...
export default function FixedAssetForm({ asset, onClose }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      let savedAsset;
      if (asset?.id) {
        savedAsset = await FixedAsset.update(asset.id, calculatedData);
      } else {
        savedAsset = await FixedAsset.create(calculatedData);
        ...
        if (!asset && data.acquisition_cost > 0 && data.asset_account_id) {
          ...
          if (equityAccount) {
            ...
            if (assetAccount) {
              await Account.update(assetAccount.id, {
