...
import { RecordLock, AuditLog, RecordVersion } from '@/api/entities';
    ...
    try {
      ...
      for (const lock of locks) {
        ...
        if (expiresAt > now && lock.session_id !== sessionId) {
          ...
          await RecordLock.update(lock.id, { is_active: false });
        }
      }
...
export function useRecordVersion() {
  ...
  const saveVersion = useCallback(async (entityName, recordId, recordData, changeDescription = '') => {
    ...
    try {
      ...
      const versions = await RecordVersion.list({ filters: {
        company_id: currentCompany.id,
        entity_name: entityName,
        record_id: recordId
      }, orderBy: 'version_number', ascending: false });
  ...
  const getVersions = useCallback(async (entityName, recordId) => {
    ...
    try {
      return await RecordVersion.list({ filters: {
        company_id: currentCompany.id,
        entity_name: entityName,
        record_id: recordId
      }, orderBy: 'version_number', ascending: false });
